...
type BlockFooter struct {
	...
	// aggregated_signature is a BLS12-381 aggregate signature (96 bytes, G2
	// compressed) over the COMMIT vote of this block. Populated once BLS
	// signature aggregation is activated (IIP-52); empty for legacy blocks.
	AggregatedSignature []byte `protobuf:"bytes,3,opt,name=aggregated_signature,json=aggregatedSignature,proto3" json:"aggregated_signature,omitempty"`
	// signer_bitmap encodes which delegates of the current epoch contributed to
	// aggregated_signature. Bit i (LSB-first within each byte) corresponds to
	// delegate i in the epoch's delegate list. Empty for legacy blocks.
	SignerBitmap  []byte `protobuf:"bytes,4,opt,name=signer_bitmap,json=signerBitmap,proto3" json:"signer_bitmap,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *BlockFooter) GetAggregatedSignature() []byte {
	if x != nil {
...
func (x *BlockFooter) GetSignerBitmap() []byte {
	if x != nil {
...
const file_proto_types_blockchain_proto_rawDesc = "" +
	...
	"\rexcessBlobGas\x18\f \x01(\x04R\rexcessBlobGas\"\xdc\x01\n" +
	"\vBlockFooter\x12;\n" +
	...
	"\x14aggregated_signature\x18\x03 \x01(\fR\x13aggregatedSignature\x12#\n" +
	"\rsigner_bitmap\x18\x04 \x01(\fR\fsignerBitmap\"9\n" +
	"\tBlockBody\x12,\n" +
